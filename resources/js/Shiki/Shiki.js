...
let theme = args[2] || 'one-dark-pro'

const languagesToLoad = allLanguages.filter(
  (lang) => lang.id === language || (lang.aliases && lang.aliases.includes(language))
)
...
;(function loadEmbeddedLangsRecursively() {
  languagesToLoad.forEach(function (language) {
    ...
    embeddedLangs.forEach(function (languageKey) {
      if (
        languagesToLoad.find((lang) => lang.id === languageKey || (lang.aliases && lang.aliases.includes(languageKey)))
      ) {
        return
      }

      languagesToLoad.push(
        allLanguages.find((lang) => lang.id === languageKey || (lang.aliases && lang.aliases.includes(languageKey)))
      )
